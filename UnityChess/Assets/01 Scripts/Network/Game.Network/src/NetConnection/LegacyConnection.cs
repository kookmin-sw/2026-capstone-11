// 유니티 포팅 불가 이슈로 Channel<byte[]> 기반에서 ConcurrentQueue로 변경
...
//                 await tcp.ConnectAsync(ip, portNum, ct);
...
//                     await _stream.ReadExactlyAsync(sizeBuffer, 0, sizeBuffer.Length, token);

//                     int size = BitConverter.ToInt32(sizeBuffer);
//                     byte[] dataBuffer = new byte[size];
//                     await _stream.ReadExactlyAsync(dataBuffer, 0, dataBuffer.Length, token);
