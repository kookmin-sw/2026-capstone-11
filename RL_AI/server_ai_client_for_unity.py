...
# ======================================================
# [변경] server_protocol_for_unity 로 교체
#   (Unity 연결 흐름용 핸들러 ID·SimpleReq/Rsp 헬퍼 추가)
# ======================================================
from RL_AI.server_protocol_for_unity import (
    FLAG_NONE,
    FLAG_QUERY,
    FLAG_RESPOND,
...
class ServerAiClient:
    def __init__(
        ...
        *,
        ...
        self.deck = str(deck).strip().lower()
    ...
    def _build_agent(self):
        ...
        if self.mode in {"rl", "belief_mcts", "mcts"}:
            if not self.model_path:
                raise ValueError("mode=rl/belief_mcts/mcts requires --model-path")
            rl_agent = _load_rl_agent(model_path=self.model_path, device=self.device, seed=self.seed)
            if self.mode in {"belief_mcts", "mcts"}:
    ...
    def run(self) -> None:
        with socket.create_connection((self.host, self.port)) as sock:
            ...
            # ======================================================
            # [변경] Unity 서버 연결 흐름 (순차 실행, 각 단계 사이 1초 대기)
            # ======================================================
            ...
            self._log(f"PeerEntrance rsp: {rsp.payload_text()}")

            time.sleep(1.0)

            # 2. GameDataRegister — SimpleReq 포맷으로 게임 데이터 전송
            _DECKS = {
                "orange":    '["Or_L", "Or_B", "Or_R", "Or_N", "Or_P", "Or_P", "Or_P"]',
                "charlotte": '["Cl_L", "Cl_B", "Cl_R", "Cl_N", "Cl_P", "Cl_P", "Cl_P"]',
            }
            game_data = _DECKS.get(self.deck, _DECKS["orange"])
            self._log(f"deck={self.deck}: {game_data}")
            send_packet(sock, FLAG_QUERY, HANDLER_GAME_DATA_REGISTER, 1, encode_simple_req(game_data))
            self._log("sent GameDataRegister")
            # SimpleRsp 응답 수신 (대기 없이 즉시)
            rsp = recv_packet(sock)
            rsp_data = decode_simple_rsp(rsp.payload)
            self._log(f"GameDataRegister rsp: {'Accepted' if rsp_data['result'] == 0 else 'Denied'}, msg={rsp_data['msg']}")

            time.sleep(1.0)

            # 3. GameReady — SimpleReq 포맷으로 "Ready" 전송 (이후 즉시 게임 시작)
            send_packet(sock, FLAG_QUERY, HANDLER_GAME_READY, 2, encode_simple_req("Ready"))
            self._log("sent GameReady → game starting")

            # ======================================================
            # [유지] 게임 메인 루프 (내부 로직 변경 없음)
            # ======================================================
            while True:
                try:
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--mode", choices=["rl", "belief_mcts", "mcts", "greedy", "random"], default="rl")
    parser.add_argument("--model-path", help="model checkpoint path for rl/belief_mcts/mcts mode")
    parser.add_argument("--player-name", default="AI Player")
    parser.add_argument("--seed", type=int, default=None)
    parser.add_argument("--device", default="auto")
    parser.add_argument("--card-data-path", default=None)
    parser.add_argument("--deck", choices=["orange", "charlotte"], default="orange", help="deck to register (default: orange)")
