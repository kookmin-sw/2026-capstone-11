...
{
    public class SessionData : INetReceiveEventHandler
    {
        public int HandlerId => NetEventHandlerId.Constant.GameDataRegister;
        private INetAPI _net;
        private SessionEvents _events;
        private SessionRouter _router;

        public SessionData(INetAPI Net, SessionEvents Events, SessionRouter Router)
        {
            _net = Net;
            _events = Events;
            _router = Router;
        ...
        public void OnQuery(ConnId connId, int queryNum, byte[] raw)
        {
            try
            {
                if (_router.TryRoute(connId, out string name) && _events.OnSessionPlayerDataUpdate != null)
                {
                    PacketReader reader = new(raw);
                    var req = SimpleReq.Codec.Read(ref reader);

                    SimpleRsp rsp = _events.OnSessionPlayerDataUpdate(name, req);
